...
* A `Bureaucrat` class is created with:
  * A constant `name`.
  * A `grade` (WARNING: Pay attention to the ranges, they can cause confusions).
...
* For both attributes, the following methods are implemented:
  * `getName()`
  * `getGrade()`
* Two member functions are implemented to increment or decrement the bureaucrat's grade (The subject doesn't give names for the functions, so you are free to choose your own. In this case: `GraDecrement` and `GraIncrement`):
* An overload of the insertion (<<) operator is implemented to print the output in a specific format: `<name>, bureaucrat grade <grade>`.
...
This exercise introduces **exception handling in C++**, the main concepts are: `throw`, `try`, `catch`. And also teaches us the inheritance from `std::exception`.
...
* A `Form` class is added with:
    * A *constant* `name`.
    * A *boolean* indicating whether the form is signed (it must not be signed when constructed).
    * A *constant* grade required to *sign* the form.
    * A *constant* grade required to *execute* the form.
* All attributes are *private*.
* The form always starts as *unsigned*.
...
* Getters are implemented for each one of the new attributes: `getFormName`, `getSignedStat`, `getSigGrade` and `getExGrade`.
* The insertion operator is overloaded (`<<`) to display *all the form's information*.

### What can we learn about this exercise?:

This exercise builds on the exception system from `ex00` and introduces **interaction between classes through exceptions**. We will also learn to manage a strict validation between classes.
...
* All the files from the previous exercise are copied here.
* `Form` is renamed to `AForm` and turned into an **abstract class**.
* The form attributes are kept *private*.
* The following concrete form classes are created, and have these characteristics:
    * `ShrubberyCreationForm`
        * sign grade is *145*
        * execute grade is *137*
        * When executed, it creates a file named `<target>_shrubbery` in the current working directory and writes several ASCII trees into the file (you are free to choose the type of tree you prefer).
    * `RobotomyRequestForm`
        * sign grade is *72*
        * execute grade is *45*
        * When executed, it:
            * Makes drilling noises.
            * Display a message announcing that the robotomy has been successful or has failed (the chances that the first or the second message is displayed are 50% each one).
    * `PresidentialPardonForm`
        * sign grade is *25*
        * execute grade is *5*
        * When executed, it informs that the target has been pardoned by: `Zaphod Beeblebrox`
    * Each concrete form receives only one constructor parameter, it is the target of the form.
* The function `void execute(Bureaucrat const &executor) const` is added to `AForm`. This function is used on the other forms to execute on each one its concrete actions, but before one Form is executed, it is checked that the form *has been signed*, and the bureaucrat has a *sufficiently high grade*. If either requirement is not satisfied, an appropriate exception is thrown. The specific action of each form is inside them, not in the base class, although, the requirements are validated in the base class through the function `checkGrade`.
* The function `void executeForm(AForm const &form) const` is added to `Bureaucrat`, and tries to execute the form, if it is successful it prints: `<bureaucrat> executed <form>`, if not it prints the proper exception.

### What can we learn about this exercise?:

This is the exercise where several previous concepts come together, above all **Abstract classes** and **Pure virtual functions**. The main challenge is the inheritance of the forms, maintaining its concrete actions and validations aligned with the requirements of the subject. Keeping in mind always the exception cases.
...
* An `Intern` class is added.
    ...
    * But it has implemented the function `AForm *makeForm(std::string formName, std::string target)`, it has:
        ...
        * A return value which consists of a pointer to an `AForm` object corresponding to the form *name* passed as parameter.
    * The intern prints `Intern creates <form>`, if the name provided exists, if not it prints an error message.
* In this exercise, it is important to avoid solutions as if / else if... The best solution is the one that uses an *array of pointers to functions* (as we can see in the exercise of this repository).

### What can we learn about this exercise?:

This exercise introduces a simple **factory-like pattern**. Instead of the rest of the program having to know how every form is constructed, that responsibility is centralized inside `Intern`. The exercise also teaches us how to use other comparison methods that aren't switch or if/else if.
